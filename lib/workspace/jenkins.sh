...
if [ ! -e $_JENKINS_JOB_CONFIGURATION_FILE ]; then
...
jenkins -url $_JENKINS_URL -username ${_JENKINS_SECRET_KEY}/username -password ${_JENKINS_SECRET_KEY}/password -j $_JENKINS_JOB_NAME $_JENKINS_SUB_COMMAND
